...
void LCD_nokia_init(void)
{
	...
	GPIO_PinInit(GPIO_DATA_OR_CMD_PIN, DATA_OR_CMD_PIN, &led_config);  //PTC5 - DC
	GPIO_PinInit(GPIO_RESET_PIN, RESET_PIN, &led_config);             //PTC7 - RST PIN
...
void LCD_nokia_write_byte(uint8_t data_or_command, uint8_t data)
{
	...
	masterXfer.configFlags = kDSPI_MasterCtar0 | kDSPI_MasterPcs0 | kDSPI_MasterPcsContinuous; //IMPORTANTE, se señala el ctar a utilizar y el chip select a utilizar
